...
export default function Dashboard() {
    ...
    const fetchBoards = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/boards/user/${user.id}`,
        );
        setBoards(res.data);
      } catch (err) {
  ...
  const renameBoard = async (_id: string, _title: string) => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/boards/${_id}`,
        { title: _title, userId: user.id },
      );
  ...
  const toggleStar = async (e: any, _id: string, _starred: boolean) => {
    ...
    try {
      const res = await axios.put(
        `${import.meta.env.VITE_API_URL || "http://localhost:5000"}/api/boards/${_id}/star`,
        { userId: user.id },
      );
      setBoards(boards.map((b) => (b._id === _id ? res.data : b)));
    } catch (err) {
      ...
      <div className="flex">
        ...
        <aside className="hidden md:flex flex-col w-64 shrink-0 min-h-screen border-r-2 border-[#2B2B2A]/10 px-6 py-8">
          <Link to="/" className="flex items-center gap-3 mb-10 cursor-pointer">
            ...
            <span className="font-doodle text-2xl font-bold tracking-tight">
              DrawForge
            </span>
          </Link>
          ...
          <Link
            to="/board/new"
            className="doodle-btn w-full flex items-center justify-center gap-2 bg-[#FF6B6B] text-[#2B2B2A] py-3 border-2 border-[#2B2B2A] font-doodle font-bold text-xl shadow-[3px_3px_0_#2B2B2A] hover:shadow-[1px_1px_0_#2B2B2A] hover:translate-x-[2px] hover:translate-y-[2px] transition-all mb-8"
          >
            <Plus size={20} />
            New board
          </Link>
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 text-lg text-left transition-colors ${
                    isActive
                      ? "bg-[#FFC53D]/25 border-2 border-[#2B2B2A]"
                      : "border-2 border-transparent hover:bg-[#2B2B2A]/5"
                  } doodle-input`}
                >
                  <Icon size={18} />
                  {item.label}
                  ...
                  <div className="flex flex-col gap-2 pb-2 border-b-2 border-[#2B2B2A]/10">
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="text-sm bg-[#4FC1CF] text-[#2B2B2A] px-3 py-1 rounded-xl border-2 border-[#2B2B2A] font-bold shadow-[2px_2px_0_#2B2B2A] hover:shadow-[1px_1px_0_#2B2B2A] hover:translate-x-[1px] hover:translate-y-[1px] transition-all"
                      >
                        Save
                      </button>
                      <button
                        onClick={() => setIsEditingName(false)}
                        className="text-sm text-[#5b5b58] px-2 py-1 hover:text-[#FF6B6B] font-bold"
                      >
                        Cancel
                      </button>
                    </div>
        ...
        <main className="flex-1 px-6 md:px-10 py-8 max-w-6xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-10">
            <div>
              ...
              <p className="text-[#5b5b58] text-lg mt-1">
                You've got {boards.length} boards waiting for you.
              </p>
            </div>
            ...
            <div className="relative w-full sm:w-72">
              <Search
                size={18}
                className="absolute left-4 top-1/2 -translate-y-1/2 text-[#8a8a86]"
              />
              <input
                type="text"
                value={searchQuery}
          ...
          <div className="grid lg:grid-cols-2 gap-8 mb-14 mt-4">
            <Link
              to="/board/new"
              className="relative doodle-card wobble-1 bg-white border-2 border-[#2B2B2A] shadow-[6px_6px_0_#FF6B6B] p-8 text-left hover:-translate-y-1 transition-transform block"
            >
              <div className="w-14 h-14 rounded-full bg-[#FF6B6B]/20 border-2 border-[#2B2B2A] flex items-center justify-center mb-6">
                <Plus size={24} />
              </div>
              <h3 className="font-doodle text-3xl font-bold mb-2">
                Blank board
              </h3>
              <p className="text-[#5b5b58] text-lg max-w-sm">
                Start from a fresh canvas and let your imagination run wild.
              </p>
            </Link>
            ...
            <div className="relative doodle-card bg-white border-2 border-[#2B2B2A] shadow-[6px_6px_0_#9B7EDE] p-8">
              ...
              <div className="flex flex-col h-full justify-between gap-6">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-doodle text-3xl font-bold mb-1">
                      Create a room
                    </h3>
                    <p className="text-[#5b5b58]">
                      Spin up a live session and get a shareable code — perfect
                      for a quick jam with the team.
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <Link
                    to="/board/new"
                    className="doodle-btn flex items-center justify-center gap-2 bg-[#9B7EDE] text-[#2B2B2A] px-6 py-3 border-2 border-[#2B2B2A] font-doodle font-bold text-xl shadow-[3px_3px_0_#2B2B2A] hover:shadow-[1px_1px_0_#2B2B2A] hover:translate-x-[2px] hover:translate-y-[2px] transition-all shrink-0"
                  >
                    <Plus size={18} />
                    Create room
                  </Link>
                  <div className="flex flex-1 border-2 border-[#2B2B2A] doodle-input bg-white overflow-hidden">
                    ...
                    <button
                      onClick={() =>
                        roomCode.trim() && navigate(`/board/${roomCode.trim()}`)
                      }
          ...
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-doodle text-3xl font-bold capitalize">
              {active === "boards" ? "My Boards" : active}
            </h2>
                ...
                >
                  ...
                  >
                    <div className="doodle-thumb overflow-hidden border-2 border-[#2B2B2A] mb-4 h-28">
                      <ThumbnailPattern
                        pattern={getPattern(b._id)}
                        color={getColor(b._id)}
                      />
                    </div>
                    <div className="flex items-start justify-between gap-2 px-1">
                      <div className="w-full mr-2">
                        {renamingId === b._id ? (
                          ...
                          <h3 className="font-doodle text-2xl font-bold leading-tight truncate mb-1">
                            {b.title}
                          </h3>
                        )}
                  ...
                  <button
                    onClick={(e) =>
                      toggleStar(
                        e,
                        b._id,
                        user ? b.starredBy?.includes(user.id) : false,
                      )
                    }
                  ...
                  >
                    <Star
                      size={14}
                      className={
                        user && b.starredBy?.includes(user.id)
                          ? "fill-[#2B2B2A] text-[#2B2B2A]"
                          : "text-[#8a8a86]"
                      }
                    />
                  </button>
                  ...
                  <div className="absolute right-4 top-[152px] z-10">
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setOpenDropdownId(
                          openDropdownId === b._id ? null : b._id,
                        );
                      }}
                    ...
                    >
                      <MoreHorizontal
                        size={18}
                        className="text-[#8a8a86] hover:text-[#2B2B2A]"
                      />
                    </button>
